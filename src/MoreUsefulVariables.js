...
serein.registerPlugin('更多有用的变量', 'v1.0', 'Zaitonn', '');

if (
    System.Environment.Version.Major == 6 &&
    !System.IO.File.Exists('./plugins/deviceinfo/PreLoadConfig.json')
) {
...
function updateInfos() {
    ...
    for (let index = 0; index < allDirves.length; index++) {
        ...
        serein.setVariable(
            `deviceinfo_drivetype_${drive.Name[0]}`,
            DriveType[drive.DriveType] || '未知'
        );
        serein.setVariable(
            `deviceinfo_driveformat_${drive.Name[0]}`,
            drive.DriveFormat
        );
        serein.setVariable(
            `deviceinfo_free_${drive.Name[0]}`,
            getStrOfSpace(drive.TotalFreeSpace)
        );
        serein.setVariable(
            `deviceinfo_used_${drive.Name[0]}`,
            getStrOfSpace(drive.TotalSize - drive.TotalFreeSpace)
        );
        serein.setVariable(
            `deviceinfo_total_${drive.Name[0]}`,
            getStrOfSpace(drive.TotalSize)
        );
        serein.setVariable(
            `deviceinfo_usage_${drive.Name[0]}`,
            (100 - (drive.TotalFreeSpace / drive.TotalSize) * 100).toFixed(
                FractionDigits
            ) + '%'
        );
        serein.log(
...
空间使用率  ${(100 - (drive.TotalFreeSpace / drive.TotalSize) * 100).toFixed(
                FractionDigits
            )}%
    `
        );
...
function getStrOfSpace(num) {
    if (typeof num !== 'number' && num <= 0) return '未知';

    if (num < 1024) return `${num} B`;

    if (num < 1024 * 1024) return `${(num / 1024).toFixed(FractionDigits)} KB`;
    ...
    if (num < 1024 * 1024 * 1024 * 1024 * 1024)
        return `${(num / 1024 / 1024 / 1024 / 1024).toFixed(
            FractionDigits
        )} TB`;

    return `${(num / 1024 / 1024 / 1024 / 1024 / 1024).toFixed(
        FractionDigits
    )} PB`;
}
